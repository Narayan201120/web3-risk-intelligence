...
def save_processed(df: pd.DataFrame, output_dir: Path) -> tuple[Path, Path]:
    ...
    latest_path = output_dir / "stablecoins_latest.parquet"
    ...
    snapshot_dir = Path("data/processed_snapshots/defillama/stablecoins") / (
        f"ingestion_date={ingestion_date}"
    )
    snapshot_dir.mkdir(parents=True, exist_ok=True)

    snapshot_path = snapshot_dir / f"stablecoins_{timestamp}.parquet"
